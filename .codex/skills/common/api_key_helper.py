...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    # Step 3: Check ./.Codex/.env
    claude_env = project_dir / '.Codex' / '.env'
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3ï¸âƒ£  .Codex/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.Codex/.env", file=sys.stderr)

        print("\n4ï¸âƒ£  .Codex/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.Codex/skills/.env", file=sys.stderr)
