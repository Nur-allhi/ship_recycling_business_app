...
3. **.codex/.env file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .codex/.env
   ```

4. **.codex/skills/.env file** (shared across all Gemini skills)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .codex/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .codex/skills/your-skill/.env
   ```
