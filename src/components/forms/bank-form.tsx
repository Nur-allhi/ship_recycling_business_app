...
import { CalendarIcon, Plus, Settings2, Loader2, Link } from 'lucide-react';
...
export function BankForm({ setDialogOpen }: BankFormProps) {
  ...
  const categoryInfo = useMemo(() => (bankCategories || []).find(c => c.name === categoryName), [bankCategories, categoryName]);
  ...
  return (
    <Card className="border-0 shadow-none">
      <CardHeader>
        <CardTitle>New Bank Transaction</CardTitle>
        <CardDescription>Record a new deposit or withdrawal from a bank account.</CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit(onSubmit)}>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Controller name="bank_id" control={control} render={({ field }) => <ResponsiveSelect onValueChange={field.onChange} value={field.value} title="Select bank" placeholder="Select bank" items={bankAccountItems} />} />
