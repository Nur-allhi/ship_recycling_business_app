...
export function useAppActions() {
    ...
    const addCashTransaction = adminAction(async (tx: Omit<CashTransaction, 'id' | 'createdAt' | 'deletedAt'>) => {
    ...
    const addBankTransaction = adminAction(async (tx: Omit<BankTransaction, 'id' | 'createdAt' | 'deletedAt'>) => {
    ...
    const addStockTransaction = adminAction(async (tx: Omit<StockTransaction, 'id' | 'createdAt' | 'deletedAt'>) => {
    ...
    const addLedgerTransaction = adminAction(async (tx: Omit<LedgerTransaction, 'id' | 'deletedAt' | 'status' | 'paid_amount' | 'installments'>) => {
    ...
    const editCashTransaction = adminAction(async (originalTx: CashTransaction, updatedTxData: Partial<Omit<CashTransaction, 'id' | 'date' | 'createdAt'>>) => {
    ...
    const editBankTransaction = adminAction(async (originalTx: BankTransaction, updatedTxData: Partial<Omit<BankTransaction, 'id'| 'date' | 'createdAt'>>) => {
    ...
    const editStockTransaction = adminAction(async (originalTx: StockTransaction, updatedTxData: Partial<Omit<StockTransaction, 'id' | 'date' | 'createdAt'>>) => {
    ...
    const deleteTransaction = adminAction(useCallback(async (tableName: 'cash_transactions' | 'bank_transactions' | 'stock_transactions' | 'ap_ar_transactions', txToDelete: any) => {
    ...
    const setWastagePercentage = adminAction((percentage: number) => db.app_state.update(1, { wastagePercentage: percentage }));
    ...
    const addBank = adminAction(async (name: string) => {
    ...
    const addCategory = adminAction(async (type: 'cash' | 'bank', name: string, direction: 'credit' | 'debit') => {
        const tempId = `temp_${Date.now()}`;
    ...
    const deleteCategory = adminAction(async (id: string) => {
        await db.categories.delete(id);
        queueOrSync({ action: 'deleteCategory', payload: { id } });
    });

    const transferFunds = adminAction(async (from: 'cash' | 'bank', amount: number, date: string, bankId: string, description?: string) => {
    ...
    const setInitialBalances = adminAction(async (cash: number, bankTotals: Record<string, number>, date: Date) => {
    ...
    const addInitialStockItem = adminAction(async (item: { name: string; weight: number; pricePerKg: number }) => {
    ...
    const recordPayment = adminAction(async (contactId: string, paymentAmount: number, paymentMethod: 'cash' | 'bank', paymentDate: Date, ledgerType: 'payable' | 'receivable', bankId?: string) => {
    ...
    const recordAdvancePayment = adminAction(async (payload: { contact_id: string, amount: number, date: Date, payment_method: 'cash' | 'bank', ledger_type: 'payable' | 'receivable', bank_id?: string, description?: string }) => {
    ...
    const restoreTransaction = adminAction(useCallback(async (txType: 'cash' | 'bank' | 'stock' | 'ap_ar', id: string) => {
    ...
    const handleImport = adminAction((file: File) => {
    ...
    const handleDeleteAllData = adminAction(() => {
