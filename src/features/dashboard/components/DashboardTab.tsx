...
export function DashboardTab({ setActiveTab }: DashboardTabProps) {
  ...
  const renderValue = (value: string | number, isCurrency = true) => {
    ...
    return <div className="text-3xl md:text-4xl font-bold font-mono animate-fade-in">{formattedValue}</div>;
  };

  const renderSubtext = (value: string) => {
    if (isLoading) {
      return <Skeleton className="h-5 w-2/3 mt-1" />;
    }
    return <div className="text-sm md:text-base text-muted-foreground animate-fade-in">{value}</div>;
  }
