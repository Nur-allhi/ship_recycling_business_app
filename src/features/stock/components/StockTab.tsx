          ...
          {isMonthLoading ? (
            ...
            paginatedTransactions.map((tx: StockTransaction) => (
              <TableRow key={tx.id} data-state={selectedTxIds.includes(tx.id) && "selected"}>
                ...
                {showStockValue && <TableCell className={`text-center font-semibold font-mono ${tx.type === 'purchase' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>{formatCurrency(tx.actual_amount)}</TableCell>}
                {showActions && (
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-2">
            ...
            <Card key={tx.id} className="relative animate-fade-in">
                ...
                <CardContent className="p-4 space-y-2">
                    <div className="flex justify-between items-start">
                        <div className={`font-semibold text-lg font-mono ${tx.type === 'sale' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    ...
                    {filteredByMonth.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 border rounded-lg bg-muted/50 text-center">
                            <div className="md:border-r md:border-border md:pr-4">
                                <h4 className="font-semibold text-red-600 dark:text-red-400">Monthly Purchases</h4>
                            ...
                            <div className="border-t border-border md:border-t-0 md:pl-4 pt-4 md:pt-0">
                                <h4 className="font-semibold text-green-600 dark:text-green-400">Monthly Sales</h4>
